...
The current normal deterministic gate contains 312 unit tests, 15 CLI tests,
three directory-distribution tests, three documentation-command tests, three
replay-fixture tests, and two MCP end-to-end session tests (`tests/mcp.rs`,
which drive a real `stallhunt mcp` process over pipes through the handshake,
every tool family, and EOF shutdown). Five Linux acceptance tests are ignored
by default and run only when intentionally requested.
